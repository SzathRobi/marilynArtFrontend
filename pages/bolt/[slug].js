      ...
      <IconButton className={styles.favIconBtn} onClick={updateFav}>
        ...
        <div className={styles.imgsContainer}>
          ...
          <div className={styles.subImgContainer}>
            {product.images.map(image => (
              <CardMedia className={styles.subImg} image={image.url} />  
            ))}
          </div>
        </div>
        <CardContent className={styles.cardContent}>
          ...
          <div className={styles.mainContent}>    
            <div className={styles.contentContainer}>
              ...
              <div className={styles.categoriesContainer}>
                <Typography className={styles.categoryName} variant="h6" color="textSecondary">
                  Kategória: 
                </Typography> 
          ...
          <form className={styles.quantityForm} onSubmit={handleSubmit}>
            <Typography style={{marginRight: "1rem"}} variant="h6">Mennyiség:</Typography>
...
export async function getStaticProps({ params: { slug } }) {
    const product_res = await fetch(`https://marilynart-backend.herokuapp.com/products/?slug=${slug}`)
